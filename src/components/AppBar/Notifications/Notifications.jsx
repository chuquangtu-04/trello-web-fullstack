...
function Notifications() {
  ...
  const handleClickNotificationIcon = (event) => {
    ...
    // Khi click vào phần icon chông thông báo thì set lại trang thái newNotification về false
  ...
  useEffect(() => {
    ...
    // Lắng nghe một cái sự kiện real-time có tên là BE_USER_INVITED_TO_BOARD từ phía server trả về
    socketIoInstance.on('BE_USER_INVITED_TO_BOARD', onReceiveNewInvitation)

    // Clearn up sự kiện event  để ngăn chặn việc bị đăng ký lặp lại sự kiện ( tài liệu ở doc )
    return () => {
      socketIoInstance.off('BE_USER_INVITED_TO_BOARD', onReceiveNewInvitation)
    }
  }, [dispatch, currentUser._id])

  // Cập nhật trạng thái - status của một cái join board
            ...
            <MenuItem sx={{
              ...
              <Box sx={{ maxWidth: '100%', wordBreak: 'break-word', whiteSpace: 'pre-wrap', display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Box><strong>{notification.inviterId.username || notification.inviter.username }</strong> had invited you to join the board <strong>{notification.board.title}</strong></Box>
                </Box>
